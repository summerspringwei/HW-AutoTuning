...
class TrainerConfig(NamedTuple):
    ...
    num_epoch_full: int = 3
    num_epoch_incremental: int = 1
...
class State:
    ...
    def load(  # pylint: disable=too-many-locals
        ...
        cache_path = os.path.join(path, "cached_data.pkl")
        if os.path.exists(model_path):
            self.model.load_state_dict(torch.load(model_path))
        if os.path.exists(cache_path):
            with open(cache_path, "rb") as f:
                for group_hash, features, costs in pickle.load(f):
...
class SegmentSumMLPTrainer:
    ...
    def train_full(self):  # pylint: disable=too-many-locals
        ...
        self.scheduler = torch.optim.lr_scheduler.StepLR(
            self.optimizer,
            step_size=self.num_epoch_full // 10 if self.num_epoch_full > 10 else 1,
            gamma=0.8,
            verbose=True,
        )
