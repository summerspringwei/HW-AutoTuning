...
features = {
    'dse':
    'NumRedundantStores NumFastStores NumFastOther NumCompletePartials NumModifiedStores',
    'licm':
    'NumCreatedBlocks NumClonedBranches NumSunk NumHoisted NumMovedLoads NumMovedCalls NumLoadPromoted NumLoadStorePromoted NumMinMaxHoisted NumGEPsHoisted NumAddSubHoisted',
    ...
    'NumBranches NumSwitches NumSelects NumGuards NumTrivial NumInvariantConditionsInjected',
    'loop-simplifycfg':
    'NumTerminatorsFolded NumLoopBlocksDeleted NumLoopExitsDeleted',
    'memcpyopt':
    'NumMemCpyInstr NumMemSetInfer NumMoveToCpy NumCpyToSet NumCallSlot',
    ...
    'sroa':
    'NumAllocaPartitions NumNewAllocas NumPromoted NumLoadsSpeculated NumLoadsPredicated NumStoresPredicated NumDeleted NumVectorized',
    ...
    'loop-unroll':
    'NumUnrolled NumCompletelyUnrolled NumUnrolledNotLatch NumRuntimeUnrolled',
    'simplifycfg':
    'NumSimpl NumBitMaps NumLinearMaps NumLookupTables NumFoldValueComparisonIntoPredecessors NumFoldBranchToCommonDest NumHoistCommonInstrs NumSinkCommonInstrs NumInvokes NumInvokesMerged NumInvokeSetsFormed',
    'libcalls-shrinkwrap': 'NumWrappedOneCond NumWrappedTwoCond',
    'mem2reg':
    'NumPromoted NumLocalPromoted NumSingleStore NumDeadAlloca NumPHIInsert',
    'break-crit-edges': 'NumBroken',
    'loop-peel': 'NumPeeled',
    'lcssa': 'NumLCSSA',
    'local': 'NumRemoved NumPHICSEs',
    'build-libcalls':
    'NumReadNone NumInaccessibleMemOnly NumReadOnly NumWriteOnly NumArgMemOnly NumInaccessibleMemOrArgMemOnly NumNoUnwind NumNoCapture NumWriteOnlyArg NumReadOnlyArg NumNoAlias NumNoUndef NumReturnedArg NumWillReturn',
    'loop-simplify': 'NumNested',
    'aggressive-instcombine':
    'NumExprsReduced NumInstrsReduced NumAnyOrAllBitsSet NumGuardedRotates NumGuardedFunnelShifts',
    'instcombine':
    'NumDeadStore NumGlobalCopies NumSel NumWorklistIterations NumCombined  NumConstProp NumDeadInst  NumSunkInst  NumExpand NumFactor    NumReassoc   NumSimplified NumPHIsOfInsertValues NumPHIsOfExtractValues NumPHICSEs NumAggregateReconstructionsSimplified NegatorNumTreesNegated NegatorNumInstructionsCreatedTotal NegatorNumInstructionsNegatedSuccess',
    'globalopt':
    'NumMarked NumUnnamed NumSRA NumSubstitute NumDeleted NumGlobUses   NumLocalized  NumShrunkToBool   NumFastCallFns    NumCtorsEvaluated NumNestRemoved    NumAliasesResolved NumAliasesRemoved NumCXXDtorsRemoved NumInternalFunc NumColdCC',
    'elim-avail-extern': 'NumRemovals NumConversions NumVariables',
    'inline': 'NumInlined NumDeleted',
    'function-attrs':
    'NumMemoryAttr NumNoCapture NumReturned NumReadNoneArg NumReadOnlyArg NumWriteOnlyArg NumNoAlias NumNonNullReturn NumNoRecurse NumNoUnwind NumNoFree NumWillReturn NumNoSync NumThinLinkNoRecurse NumThinLinkNoUnwind',
    ...
    'deadargelim':
    'NumArgumentsEliminated NumRetValsEliminated NumArgumentsReplacedWithPoison',
    'vector-combine':
    'NumVecLoad NumVecCmp NumVecBO NumVecCmpBO NumShufOfBitcast NumScalarBO NumScalarCmp',
...
def read_optstats_from_cfgpath(cfg_path):
    ...
    for filename in cfg['hotfiles']:
        ...
        stats_file = os.path.join(
            cfg['tmp_dir'], fileroot,
            'IR-{}'.format(hashlib.md5(opt_str.encode('utf-8')).hexdigest()),
            fileroot + '.opt_stats')
...
def read_optstats_from_cfgpathlist(cfg_path_list, n_parallel=None):
    if n_parallel is not None:
        with Pool(n_parallel) as p:
            stats_and_pmu_list = p.map(read_optstats_from_cfgpath,
                                       cfg_path_list)
    else:
        with Pool() as p:
            stats_and_pmu_list = p.map(read_optstats_from_cfgpath,
...
def prepare_all(benchmark):
    ...
    return stats_and_pmu_list, speedup_list, time_list, y_O3
...
if __name__ == "__main__":
    all_benchmarks = [
        ...
        'consumer_tiffmedian', 'network_dijkstra', 'network_patricia',
    ...
    out_dir = 'data_perf_features'
    for benchmark in all_benchmarks:
        stats_and_pmu_list, speedup_list, time_list, y_O3 = prepare_all(
            benchmark)
        with open(f'{out_dir}/{benchmark}.pkl', 'wb') as f:
            pickle.dump((stats_and_pmu_list, speedup_list, time_list, y_O3), f)
